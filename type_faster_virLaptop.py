...
def submit():
    """Calculates the Words Per Minute that the user types if they don't want to type for 1 minute"""
    ...
    timer_label.config(text=f"Time's up! You type at {approximate_wpm:.0f} WPM with {approximate_accuracy:.0f}% accuracy!",
                       font=FONT)


    submit_button.config(text="Click here to start again!")

def restart():
    """Restarts the program and resets the variables"""
...
try:
    ...
    random_words = rwg.get_offline_words()
